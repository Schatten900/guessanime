...
function LoginForm() {
  ...
  const submiteFormData = async (e) => {
    ...
    try {
      ...
      setAlertMessage(error.message || "Something went wrong while logging");
      openAlert();
    }
  };
  ...
  return (
    <form 
    onSubmit={submiteFormData} 
    className="w-[450px] h-[450px] rounded-xl shadow-md bg-slate-100 flex flex-col items-center justify-evenly p-3">
      ...
      <div className="flex flex-col items-center w-[110%] space-y-6">
        ...
        <Input
          ...
          onChange={readInputData}
        />
        <SubmitButton onClick={submiteFormData}>
          Login
        </SubmitButton>
      </div>
